...
//    UIImage *thumbImage = [UIImage imageNamed:@"res5thumb.png"];
    ...
    NSURL *url = [NSURL URLWithString:@"http://ww4.sinaimg.cn/mw690/005Ctlmsgw1f1xz5qxid4j30ap09ymxm.jpg"];
    NSData *emoticonData = [NSData dataWithContentsOfURL:url];
    UIImage *thumbImage1 = [UIImage imageWithData:emoticonData];
    
    UIImage *thumbImage = [self scaleImage:thumbImage1 toScale:1];
    NSData *emoticonData1 = UIImageJPEGRepresentation(thumbImage, 1);
    
    [WXApiRequestHandler sendEmotionData:emoticonData1
                              ThumbImage:thumbImage
                                 InScene:_currentScene];
}
...
//                                                         ofType:@"gif"];
//    NSData *emoticonData = [NSData dataWithContentsOfFile:filePath];
    NSURL *url = [NSURL URLWithString:@"http://ww3.sinaimg.cn/mw690/005Ctlmsgw1f2fbh5kdmmg308w04q000.gif"];
    NSData *emoticonData = [NSData dataWithContentsOfURL:url];
...
RCT_EXPORT_METHOD(sendEmotionContent:(NSString *)emotionUrl
                  InScene:(NSInteger)scene) {
//    创建NSURL
    NSURL *url = [NSURL URLWithString: emotionUrl];
//    获取data
    NSData *data = [NSData dataWithContentsOfURL: url];
//    获取图像
    UIImage *image = [UIImage imageWithData: data];
//    缩略图：缩放到260px
    UIImage *thumbImage = [Image scaleToMaxSize:image maxWidth:260 maxHeight:260];
//    表情图：缩放到300px
    UIImage *emotionImage = [Image scaleToMaxSize:image maxWidth:300 maxHeight:300];
//    表情图data
    NSData *emotionData = UIImageJPEGRepresentation(emotionImage, 1);
//    获取场景
    if (scene == 0) {
        _currentScene = WXSceneSession;
    } else if (scene == 1) {
...
//    发送表情
    [WXApiRequestHandler sendEmotionData: emotionData
                              ThumbImage: thumbImage
                                 InScene: _currentScene];
}
